    ...
    const crawler = new Crawler({
      ...
      callback: function (error, res, done) {
        if (!error) {
          ...
          const videoUrl = matchData.header.videoUrl;
          let youtubeVideoChannelIsRelatedToThisCast = true;

          try {
            if (
              new URL(videoUrl).searchParams.get("ab_channel") !==
              cast.youtubeChannelSlug
            ) {
              youtubeVideoChannelIsRelatedToThisCast = false;
            }
          ...
          match = {
            ...
            youtubeVideoId: getYoutubeVideoIdFromUrl(videoUrl),
            youtubeVideoChannelIsRelatedToThisCast,
          };
        }
        done();
      },
